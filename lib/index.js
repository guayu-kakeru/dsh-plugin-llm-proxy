...
// high → thinking: {type:"enabled"} + reasoning_effort: "high"
// max  → thinking: {type:"enabled"} + reasoning_effort: "max"
// 未配置/未选择时不发送 thinking 字段，跟随网关默认（与官方 DeepSeek 适配器一致）。
...
const Config = z.object({
  ...
  defaultReasoningEffort: z.union(["off", "high", "max"]),
...
class OpenAIProxyAdapter extends LlmAdapter {
  ...
  resolveModel(provider, model) {
    ...
    const defaultEffort = connection.defaultReasoningEffort;
  ...
  async *request(options, signal, connection, apiKey, onComment) {
    const body = serializeRequest(options, { reasoningEffort: connection.defaultReasoningEffort });
