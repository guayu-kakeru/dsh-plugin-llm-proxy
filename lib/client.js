window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function ProxySettingsSection({ api, rpc }) {
      ...
      const [defaultReasoningEffort, setDefaultReasoningEffort] = React.useState("");
      ...
      React.useEffect(() => {
        if (loaded.status === "ready" && !hydrated.current) {
          ...
          setDefaultReasoningEffort(
            v.defaultReasoningEffort === "off" || v.defaultReasoningEffort === "high" || v.defaultReasoningEffort === "max"
              ? v.defaultReasoningEffort
              : "",
          );
          setModels(
      ...
      async function save() {
        ...
        try {
          ...
          const section = {
            ...
            ...(defaultReasoningEffort === "" ? {} : { defaultReasoningEffort: String(defaultReasoningEffort) }),
            models: modelsWire,
      ...
      return h(
        ...
        h(
          ...
          h(
            ...
            h("option", { value: "" }, "不设置（跟随网关默认）"),
            h("option", { value: "off" }, "Off（关闭思考）"),
            h("option", { value: "high" }, "High"),
            h("option", { value: "max" }, "Max（最强）"),
          ),
          h("span", { style: css.hint }, "DeepSeek V4 风格：off 发送 thinking:{type:disabled}；high/max 发送 thinking:{type:enabled} + reasoning_effort。不设置则不发送 thinking 字段。模型选择器里还能按会话切换。" ),
        ),
